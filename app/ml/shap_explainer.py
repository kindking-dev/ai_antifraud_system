...
class SHAPExplainer:
    ...
    def __init__(
        ...
        if len(background_data) > sample_size:
            ...
            logger.info(f"SHAP background sampled: {len(background_data)} -> {sample_size}")
        else:
        ...
        self.base_value = float(self.explainer.expected_value[1]) if isinstance(self.explainer.expected_value, list) else float(self.explainer.expected_value)
    ...
    def _load_reason_rules(self) -> List[ReasonCodeRule]:
        ...
        """
        return [
            ReasonCodeRule("tx_last_1min", 0.12, "VELOCITY_SPIKE_1M", "High transaction frequency in last 60 seconds", "positive"),
            ReasonCodeRule("tx_last_5min", 0.20, "VELOCITY_SPIKE_5M", "Elevated transaction frequency in last 5 minutes", "positive"),
            ReasonCodeRule("velocity_sec", 0.08, "RAPID_VELOCITY", "Unusually short time delta between transactions", "positive"),
            ReasonCodeRule("amount_vs_user_mean", 0.15, "AMOUNT_ANOMALY", "Amount deviates significantly from user historical mean", "positive"),
            ReasonCodeRule("log_amount", 0.10, "HIGH_AMOUNT", "Absolute transaction amount exceeds behavioral threshold", "positive"),
            ReasonCodeRule("user_avg_amount", 0.08, "LOW_AVG_AMOUNT", "User historical average amount is unusually low", "positive"),
            ReasonCodeRule("card1_target", 0.18, "CARD_BIN_RISK", "Card issuer BIN associated with elevated fraud concentration", "positive"),
            ReasonCodeRule("P_emaildomain_target", 0.12, "EMAIL_DOMAIN_RISK", "Email domain linked to synthetic or disposable accounts", "positive"),
            ReasonCodeRule("addr1_target", 0.10, "ADDRESS_RISK", "Billing address region shows anomalous fraud patterns", "positive"),
            ReasonCodeRule("hour", 0.05, "NIGHT_TRANSACTION", "Transaction occurs during low-activity hours (00:00-06:00)", "positive"),
            ReasonCodeRule("is_night", 0.05, "NIGHT_TRANSACTION", "Transaction flagged as nighttime activity", "positive"),
        ]
    ...
    def explain(self, features_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if features_df.shape[0] != 1:
            raise ValueError("SHAPExplainer.explain() expects exactly one sample per call")
        ...
        try:
            ...
            raw_impacts = {
                feat: float(val) 
                for feat, val in zip(self.feature_names, shap_vals[0])
            }
